...
public class LatestSeasonSelectorTest extends FileSystemEnvironment {

    private LatestSeasonSelector selector;

    @Before
    public void setUp() throws Exception {
        Arguments args = ArgsParser.parse(arg("Scrubs", "s$"));
    ...
    public void getSelector() {
        assertTrue("s$".matches(selector.getSelector()));
    }

    @Test
    public void findMatches() throws IOException {
        List<EpisodeMatch> actualMatches = selector.findMatches();
        assertEquals(MockFileSystem.NUM_EPISODES, actualMatches.size());
        List<EpisodeMatch> expectedMatches = MockFileSystem.getFullSeasonEpisodeMatches("Scrubs", MockFileSystem.NUM_SEASONS, MockFileSystem.NUM_SEASONS);
